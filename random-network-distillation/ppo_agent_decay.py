...
class PpoAgent(object):
    ...
    def update(self):
        ...
        # Dynamic RND decay:
        # When the agent starts receiving extrinsic rewards,
        # progressively reduce the weight of intrinsic motivation.
        if not hasattr(self, "rnd_decay"):
            self.rnd_decay = 1.0
            self.rnd_decay_min = 0.10
            self.rnd_decay_rate = 0.995
            self.rnd_decay_threshold = 0.0

        recent_ext_mean = np.mean(self.I.buf_rews_ext)

        if recent_ext_mean > self.rnd_decay_threshold:
            self.rnd_decay = max(self.rnd_decay_min, self.rnd_decay * self.rnd_decay_rate)
        ...
        self.I.buf_advs = (self.int_coeff * self.rnd_decay) * self.I.buf_advs_int + self.ext_coeff * self.I.buf_advs_ext
        ...
        info = dict(
            ...
            rewintmax_norm = self.max_int_rew, # previously rewintmax
              rnd_decay = self.rnd_decay,
              recent_ext_mean = recent_ext_mean,
              int_coeff_effective = self.int_coeff * self.rnd_decay,
    ...
    def step(self):
        ...
        for l in range(self.I.nlump):
            ...
            with dummy_context("policy_inference"):
